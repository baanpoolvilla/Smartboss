...
## ทำรอบสอง (sync ล่าสุดก่อนเลิกใช้ ChangYai) — ไม่ต้องลบของเดิมก่อน

`deploy/import-changyai.sql` **รันซ้ำได้ปลอดภัยแล้ว** — id ยกมาจาก ChangYai
ตรง ๆ ดังนั้นแถวที่เคย import ไปแล้วจะถูก **อัปเดต** ให้ตรงกับ ChangYai ล่าสุด
(สถานะ/ราคา/รูป/ฯลฯ) ส่วนแถวที่เพิ่งมีใน ChangYai จะถูก **insert** เพิ่ม —
ไม่ชนกัน ไม่ต้องลบ `maintenance.*` ของบริษัทนั้นก่อนแล้วเริ่มใหม่

ยกเว้น 2 อย่างที่ตั้งใจไม่แตะของเดิม: **ชื่อ/อีเมล/รหัสผ่านของผู้ใช้ที่เคย
import มา** (ถ้าฝ่ายบุคคลแก้ไว้แล้วที่ `/admin/users` ตามข้อ 0.5 จะไม่ถูกทับ)
และ **เลขที่เอกสาร (`WO-xxxx`/`PO-xxxx`) ของใบที่มีอยู่แล้ว** (คงที่ตลอดอายุใบ
มีแค่ใบใหม่เท่านั้นที่ได้เลขใหม่ ต่อจากเลขสูงสุดที่มีอยู่)
...
ปัญหา (งานที่เคยมีอยู่จริงไม่ควรหายไปเพราะ resync) แต่ถ้าจำเป็นต้องลบจริง ๆ
ต้องลบเอง

**สำรองก่อนเสมอ** (`sudo bash deploy/backup.sh`) เหมือนทุกครั้งที่แตะข้อมูล
ผลิต — ทั้งไฟล์ยังอยู่ใน transaction เดียว พังตรงไหนก็ย้อนกลับหมดเหมือนเดิม
