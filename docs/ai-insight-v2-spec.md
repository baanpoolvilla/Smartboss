...
### 13.3 Stat tiles (4 ช่องบนสุด) — สไตล์ตามที่ผู้ใช้เลือก
เลย์เอาต์: ตัวเลขใหญ่มุมซ้าย, ไอคอนใส่กล่องสีมุมขวาบน, label ด้านล่าง, และลิงก์ **"ดูรายละเอียด ›"** ใต้แต่ละช่อง (คั่นด้วยเส้นบางบน). ค่าเริ่มต้น 4 ช่อง: `คนควรคุยด่วน` (แดง), `งานยังไม่เสร็จ` (เหลือง), `แผนกแนวโน้มแย่ลง` (แดง), `คำแนะนำที่ได้ผลแล้ว` (เขียว). หัวการ์ดมีกราฟิกประกอบเล็กๆ (แท่ง+เส้นขาขึ้น) ที่มุมขวา
