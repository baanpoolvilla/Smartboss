...
**Label ที่แก้แล้ว (กันความกำกวม pending vs overdue):** เดิม "งานยังไม่เสร็จ" กับ "งานเลยกำหนด" อ่านคู่กันแล้วไม่ชัดว่าต่างกันตรงไหน — ทั้งคู่มาจากคนละ bucket:
- `pending` (task) → ยังไม่เสร็จ**แต่ยังไม่เลยกำหนด** → label ใหม่: **"ยังไม่เสร็จ · ในกำหนด"**
- `overdue` (task) → ยังไม่เสร็จ**และเลยกำหนดแล้ว** → label ใหม่: **"ยังไม่เสร็จ · เลยกำหนด"**
- อีก 2 การ์ด: **"คนควรคุยด่วน"** (แดง), **"รายงานยังไม่ส่ง"** (ฟ้า)
- ใช้ pattern เดียวกันนี้ถ้ามีการ์ดอื่นแตะ pending/overdue ต่อไป: `รายงานยังไม่ส่ง · ในกำหนด` (pending) vs `รายงานขาดส่ง · เลยกำหนด` (missed)
