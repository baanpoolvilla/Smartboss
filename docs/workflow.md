...
## กฎที่ต้องรักษาตั้งแต่ตอนนี้ — `actions.ts` ต้องบาง

> เจ้าของยืนยันว่า **ยังไงก็ทำแอป 100%** แต่ทำเว็บก่อนเพื่อใช้ในบริษัท
> จนกว่าฟีเจอร์จะนิ่ง — กฎข้อนี้คือสิ่งที่ทำให้วันนั้นไม่ต้องรื้อครึ่งระบบ

`actions.ts` ทำได้แค่ 4 อย่าง

```
1. ตรวจสิทธิ์
2. แกะข้อมูลจาก FormData
3. เรียกฟังก์ชันใน data/
4. revalidatePath
```

**ตรรกะ การคำนวณ การแจ้งเตือน การบันทึกคะแนน — ต้องอยู่ใน `data/`**

### วิธีตรวจว่าทำถูกไหม

ถามตัวเองว่า

> *"ถ้าเขียนสคริปต์เรียกฟังก์ชันนี้โดยไม่มีเบราว์เซอร์เลย มันทำงานครบไหม"*

- ตอบ **ใช่** ⇒ วันหน้าทำแอปแค่ห่อ HTTP บาง ๆ ทับ
- ตอบ **ไม่** ⇒ วันหน้าต้องเขียนใหม่

### ตัวอย่างที่ทำถูกแล้วในรีโปนี้

`apps/workforce-api` — NestJS แยกโปรเซส ตรรกะอยู่ใน service ส่วน HTTP เป็นแค่เปลือก
**แอปเรียกได้เลยวันนี้โดยไม่ต้องแก้อะไร** — ใช้เป็นต้นแบบได้

### ที่ยังผิดกฎอยู่ (ไม่ต้องรีบแก้)

การส่งแจ้งเตือน LINE กับการสร้างค่าใช้จ่ายอัตโนมัติของโมดูลซ่อมบำรุง
อยู่ใน `actions.ts` ไม่ได้อยู่ใน `data/`

⇒ ถ้าแอปสร้างใบงานผ่าน API **ช่างจะไม่ได้รับแจ้งเตือน**
เพราะโค้ดส่วนนั้นอยู่ในเส้นทางของเว็บเท่านั้น

ของเก่าปล่อยไว้ก่อนก็ได้ แต่**ของใหม่ควรทำให้ถูกตั้งแต่แรก**

### อย่าเพิ่งเขียน API รอไว้

หน้าตา API ต้องตามหน้าจอแอป เขียนก่อนโดยไม่รู้ว่าแอปมีกี่หน้า
จะได้ API ที่ไม่ตรงกับที่ใช้จริงแล้วต้องรื้ออยู่ดี — เขียนตอนทำหน้านั้นจริง
