...
export interface SubmissionRound {
  ...
  time: string;
  /** 0=อา..6=ส · undefined/ว่าง = ทุกวัน · ไม่ใช้ร่วมกับ dayOfMonth (มีค่าใดค่าหนึ่ง) */
  weekdays?: number[];
  /** รอบรายเดือน — ค่าวันที่ (เช่น 30 สำหรับ "Monthly Report") แทนที่จะเป็นวันใน
   * สัปดาห์ ชนะ `weekdays` เมื่อมีทั้งคู่ (ไม่ควรตั้งพร้อมกันจริง — ตัวแก้ไข
   * UI บังคับเลือกอย่างใดอย่างหนึ่ง) เดือนที่ไม่มีวันที่นี้ (เช่น 30 ในเดือน
   * กุมภาพันธ์) ตกไปที่วันสุดท้ายของเดือนแทนเสมอ ("ทุกวันที่ 30 เลย จะมีแค่
   * เดือนกุมภาที่ 28-29 แล้วแต่ปี") ดู roundRunsOnDay ในตัวคำนวณจริง */
  dayOfMonth?: number;
