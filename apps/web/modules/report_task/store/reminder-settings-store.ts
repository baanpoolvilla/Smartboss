...
export interface ReportSubmissionLockSettings {
   ...
   * company-wide, unless a room opted itself out (see
   * `hardCutoffExemptFromGlobal`). OFF: each room decides on its own (unset
   * = never locks, same as before this feature existed). Separate from the
   * *reminder* lead-time above and from a room's submission-round times —
   * those only ever badge a post "ส่งช้า"; this is the only thing that can
   * actually block the submit button. */
