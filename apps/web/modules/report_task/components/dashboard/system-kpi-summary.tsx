...
export function SystemKpiSummary() {
  ...
  const data = useMemo(() => {
    ...
    const groups: KpiGroup[] = (["lateDone", "onTime", "overdue", "pending"] as const).map((key) => ({
      key,
      // "เลยกำหนด"/"รอดำเนินการ" แทน "ยังไม่เสร็จ เลยกำหนด"/"ยังไม่เสร็จ ในกำหนด" เดิม —
      // อ่านเร็วๆ คำว่า "ยังไม่เสร็จ" ชนกับ "ในกำหนด" แล้วอ่านสลับเป็น "เสร็จตรงเวลา" ได้
      // ("ตัดคำว่า 'ยังไม่เสร็จ' ออก...เปลี่ยนเป็นชื่อสถานะสั้นๆ แทน") ไม่ต้องพูดซ้ำด้วย
      // เพราะอยู่ในครึ่งขวา "ยังไม่เสร็จ" ของกราฟอยู่แล้ว (ดูป้ายด้านบนกราฟ)
      label: { overdue: "เลยกำหนด", pending: "รอดำเนินการ", lateDone: "เสร็จช้าแต่เลยกำหนด", onTime: "เสร็จ ตรงเวลา" }[key],
                ...
                <div className="flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2">
                  ...
                  <p className="text-[12px] text-[var(--ink)]">
                    ...
                    <span className="text-[var(--ink-soft)]">
                      {issueSuggestion(data.mainIssue.key, data.mainIssue.people.length, data.mainIssue.people[0]?.count ?? data.mainIssue.count)}
                    </span>
                  </p>
                </div>
              </div>
