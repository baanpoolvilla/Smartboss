  ...
  return (
    <HrPage
      ...
      load={async () => {
        ...
        const [employments, board, timeline, shifts, todayAssignments, todayLeave] =
          await Promise.all([
        ...
        const onLeaveToday = new Set(
          (todayLeave?.items ?? []).map((l) => l.employment_id),
        );
                ...
                <DataTable head={["พนักงาน", "เข้างาน", "สแกนล่าสุด", "ตามกะ", "สถานะ"]}>
                  {missingRows.map((m) => (
                    <tr key={m.employment_id} className="hover:bg-(--bg-soft)">
                      ...
                      <Td>
                        {m.status === "ON_LEAVE" ? (
                          <Pill tone="var(--tone-info)">ลา</Pill>
                        ) : m.status === "REST_DAY" ? (
