...
import { listFileVersions, listShareLinks, listMovableFolders } from "@/modules/company-files/data/files";
...
export default async function CompanyFileDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <AppScaffold title={file.name} width="max-w-3xl" backHref={file.folderId ? `/company-files?folder=${file.folderId}` : "/company-files"}>
      <FileDetail file={file} versions={versions} shareLinks={shareLinks} uploaderNames={uploaderNames} movableFolders={movableFolders} />
    </AppScaffold>
  );
}
