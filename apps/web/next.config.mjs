...
const nextConfig = {
   ...
   *
   * 6mb เคยพอสำหรับฟอร์มที่รับรูปเดียว แต่ฟอร์ม PR/PO ("ยืนยันดำเนินการ" /
   * "รับของ") รับได้หลายรูปพร้อมกัน (ImagePickerRow's แกลเลอรี่ multiple —
   * po-actions.tsx) — มือถือรุ่นใหม่ถ่ายรูปเดียวก็ 3-8MB แล้ว แนบสัก 2-3 รูป
   * ก็ทะลุ 6mb ได้ง่าย ๆ แล้วเจอ 500 หน้าดำแบบเดียวกับตอนรูปโปรไฟล์เดิม
   * ("รอกข้อมูล ใส่รูป กดบันทึก ขึ้นดำ") ตั้งสูงกว่าเดิมพอให้แนบได้หลายรูปจริง
   * โดยยังเผื่อ overhead ของ multipart encoding
   */
  experimental: {
    serverActions: {
      bodySizeLimit: "25mb",
